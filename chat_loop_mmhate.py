...
def extract_frames_for_segments(video_path, num_segments=4, frames_per_segment=4):
    """
    ...
    frames_per_segment = min(frames_per_segment, segment_length)  # Ensure we don't request more frames than available in a segment

    segments = []
    for i in range(num_segments):
        # Calculate start and end frame indices for the current segment
        start_frame = i * segment_length
        end_frame = (i + 1) * segment_length if i < num_segments - 1 else total_frames

        # Calculate evenly spaced frame indices within the segment
        frame_indices = np.linspace(start_frame, end_frame - 1, frames_per_segment, dtype=int)
        frames = vr.get_batch(frame_indices).asnumpy()

        segments.append({
            "text": f"Segment {i + 1}: No audio detected",
            "start": start_frame / vr.get_avg_fps(),  # Convert frame index to time
            "end": end_frame / vr.get_avg_fps(),      # Convert frame index to time
            "frames": frames
        })
...
def process_video_for_llava(video_path, max_segments=None):
    ...
    if not transcript_segments:
        ...
        segments = extract_frames_for_segments(video_path, num_segments=4, frames_per_segment=4)
        return segments
...
if __name__ == "__main__":
    ...
    output_tsv = "/home/itfelicia/MultiHateClip/ablation.tsv"  # Path to save the results
    video_ids = ["0EbjPffoUmE", "bVQcoSLoapc", "AaNb5fHAZxc", "i5E3SJKTD5g", "68gvhiQVzDI", "AtE-wavoBkY", "AdBmBcp7ni0", "IahCMMYyy4w", "EEC98aHSgIY", "Z4xwQ4SRUAs", "qFnOYeOmPFQ", "g8VZhiDSkXI", "ZI_jbZWzW1A", "h_wKRDyoG_c", "58XKDvS0zE0", "xqgjkn3uh0w", "LYrlk0uy6WM", "IQEqNUqtfqg", "43wAmNR5EBM", "zvqFZAYn4fg", "pKeIcylm_B0", "16g5DLajtKc", "-cgm2EZcLC8", "IFP0l5EUO70", "6EL3gDMlve4", "UeRqjzlI4jY", "k9OtaMbK0Ac", "_qldaPBgkk0", "R3Xt1__7TwQ", "uPJtlBAOT_U", "qLa59rRqxaE", "FuXd3OYtlQU", "lzKJ_AWegCc", "CdeJBx1r6qY", "RAGvPqMoW-4", "ulze2hXRwJg", "cXRgVEENkPA", "ndqGWFON2Dk", "01ygFLVdj8s", "jEdH8-AZ3aU", "LMOHDiG5XQg"]
