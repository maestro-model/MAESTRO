...
def extract_audio_transcript(video_path, audio_path):
    """
    Extract the transcript from a video and save the audio file.
    """
...
def process_video_for_llava(video_path, audio_path, max_segments=None):
    """
    ...
    transcript_segments = extract_audio_transcript(video_path, audio_path)
...
# Define paths
video_folder = "/home/itfelicia/demo_videos"
output_excel_path = "/home/itfelicia/demo_videos/hatefulness_analysis_results.xlsx"

# Function to validate video files
def is_valid_video(video_path):
    return os.path.exists(video_path) and os.path.isfile(video_path)

# Generate .wav file from video
def generate_wav_file(video_path):
    ...
    try:
        ...
        print(f"Error generating .wav file for {video_path}: {e}")
        return None

def run_full_framework(video_path):
    try:
        ...
        audio_path = generate_wav_file(video_path)
        if not audio_path:
            print(f"Failed to generate audio for {video_path}. Skipping.")
            return None

        # Process video for transcript and frames
        idea_frame_pairs = process_video_for_llava(video_path, audio_path)
        print(f"Processed {len(idea_frame_pairs)} segments for {video_path}.")

        # Add further processing like YOLO, LLaVA, etc., here
...
def main():
    ...
    for file_name in os.listdir(video_folder):
        if file_name.endswith(".mp4"):
            ...
            print(f"Starting processing for video: {file_name}")
            final_decision = run_full_framework(video_path)
            results.append({
                "Video Name": file_name,
                "Final Decision": final_decision or "Error during processing"
            })
